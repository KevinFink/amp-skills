...
const cases: Array<{ command: string; expected: ExpectedDecision }> = [
	...
	{ command: 'terraform init', expected: 'allow' },
	{ command: 'terraform init -backend=false', expected: 'allow' },
	{ command: 'terraform validate', expected: 'allow' },
	{ command: 'terraform plan -out=plan.tfplan', expected: 'allow' },
	{ command: 'terraform show plan.tfplan', expected: 'allow' },
	{ command: 'terraform output -json', expected: 'allow' },
	{ command: 'terraform state list', expected: 'allow' },
	{ command: 'terraform state show aws_s3_bucket.example', expected: 'allow' },
	{ command: 'terraform workspace show', expected: 'allow' },
	{ command: 'npx html-validate index.html src/page.html', expected: 'allow' },
	{ command: "sed -n '1,80p' README.md", expected: 'allow' },
	{ command: "sed 's#.*/##'", expected: 'allow' },
	{ command: "find db/migrations_yoyo -maxdepth 1 -type f | sed 's#.*/##' | sort -V | tail -80", expected: 'allow' },
	{ command: "sed -n '1,140p' db/migrations_yoyo/0016_outreach_smartlead_registry.sql 2>/dev/null", expected: 'allow' },
	{ command: "sed -n '1,140p' db/migrations_yoyo/0016_outreach_smartlead_registry.sql >/dev/null 2>&1", expected: 'allow' },
	...
	{ command: "sed -n '1,80p' README.md > out.txt", expected: 'ask' },
	{ command: "sed -n 's/a/b/w out.txt' README.md", expected: 'ask' },
	{ command: "sed 's/a/b/e' README.md", expected: 'ask' },
